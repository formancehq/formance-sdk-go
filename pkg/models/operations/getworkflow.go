// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 89f57972c5f6

package operations

import (
	"github.com/formancehq/formance-sdk-go/v3/pkg/models/orchestration"
	"net/http"
)

type GetWorkflowRequest struct {
	// The flow id
	FlowID string `pathParam:"style=simple,explode=false,name=flowId"`
}

func (g *GetWorkflowRequest) GetFlowID() string {
	if g == nil {
		return ""
	}
	return g.FlowID
}

type GetWorkflowResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// The workflow
	GetWorkflowResponse *orchestration.GetWorkflowResponse
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
}

func (g *GetWorkflowResponse) GetContentType() string {
	if g == nil {
		return ""
	}
	return g.ContentType
}

func (g *GetWorkflowResponse) GetGetWorkflowResponse() *orchestration.GetWorkflowResponse {
	if g == nil {
		return nil
	}
	return g.GetWorkflowResponse
}

func (g *GetWorkflowResponse) GetStatusCode() int {
	if g == nil {
		return 0
	}
	return g.StatusCode
}

func (g *GetWorkflowResponse) GetRawResponse() *http.Response {
	if g == nil {
		return nil
	}
	return g.RawResponse
}
