// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/formancehq/formance-sdk-go/v3/pkg/models/payments"
	"net/http"
)

var V3ListPoolsServerList = []string{
	"http://localhost:8080/",
}

type V3ListPoolsRequest struct {
	RequestBody map[string]any `request:"mediaType=application/json"`
	// Parameter used in pagination requests. Set to the value of next for the next page of results. Set to the value of previous for the previous page of results. No other parameters can be set when this parameter is set.
	//
	Cursor *string `queryParam:"style=form,explode=true,name=cursor"`
	// The number of items to return
	PageSize *int64 `queryParam:"style=form,explode=true,name=pageSize"`
}

func (v *V3ListPoolsRequest) GetRequestBody() map[string]any {
	if v == nil {
		return nil
	}
	return v.RequestBody
}

func (v *V3ListPoolsRequest) GetCursor() *string {
	if v == nil {
		return nil
	}
	return v.Cursor
}

func (v *V3ListPoolsRequest) GetPageSize() *int64 {
	if v == nil {
		return nil
	}
	return v.PageSize
}

// #region class-body-v3listpoolsrequest
// #endregion class-body-v3listpoolsrequest

type V3ListPoolsResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// OK
	V3PoolsCursorResponse *payments.V3PoolsCursorResponse
}

func (v *V3ListPoolsResponse) GetContentType() string {
	if v == nil {
		return ""
	}
	return v.ContentType
}

func (v *V3ListPoolsResponse) GetStatusCode() int {
	if v == nil {
		return 0
	}
	return v.StatusCode
}

func (v *V3ListPoolsResponse) GetRawResponse() *http.Response {
	if v == nil {
		return nil
	}
	return v.RawResponse
}

func (v *V3ListPoolsResponse) GetV3PoolsCursorResponse() *payments.V3PoolsCursorResponse {
	if v == nil {
		return nil
	}
	return v.V3PoolsCursorResponse
}

// #region class-body-v3listpoolsresponse
// #endregion class-body-v3listpoolsresponse
