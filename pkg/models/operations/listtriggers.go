// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 04bb46e57851

package operations

import (
	"github.com/formancehq/formance-sdk-go/v3/pkg/models/orchestration"
	"net/http"
)

type ListTriggersRequest struct {
	// search by name
	Name *string `queryParam:"style=form,explode=true,name=name"`
}

func (l *ListTriggersRequest) GetName() *string {
	if l == nil {
		return nil
	}
	return l.Name
}

type ListTriggersResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// List of triggers
	ListTriggersResponse *orchestration.ListTriggersResponse
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
}

func (l *ListTriggersResponse) GetContentType() string {
	if l == nil {
		return ""
	}
	return l.ContentType
}

func (l *ListTriggersResponse) GetListTriggersResponse() *orchestration.ListTriggersResponse {
	if l == nil {
		return nil
	}
	return l.ListTriggersResponse
}

func (l *ListTriggersResponse) GetStatusCode() int {
	if l == nil {
		return 0
	}
	return l.StatusCode
}

func (l *ListTriggersResponse) GetRawResponse() *http.Response {
	if l == nil {
		return nil
	}
	return l.RawResponse
}
