// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/formancehq/formance-sdk-go/v3/pkg/models/payments"
	"net/http"
)

var V3GetConnectorScheduleServerList = []string{
	"http://localhost:8080/",
}

type V3GetConnectorScheduleRequest struct {
	// The connector ID
	ConnectorID string `pathParam:"style=simple,explode=false,name=connectorID"`
	// The schedule ID
	ScheduleID string `pathParam:"style=simple,explode=false,name=scheduleID"`
}

func (v *V3GetConnectorScheduleRequest) GetConnectorID() string {
	if v == nil {
		return ""
	}
	return v.ConnectorID
}

func (v *V3GetConnectorScheduleRequest) GetScheduleID() string {
	if v == nil {
		return ""
	}
	return v.ScheduleID
}

// #region class-body-v3getconnectorschedulerequest
// #endregion class-body-v3getconnectorschedulerequest

type V3GetConnectorScheduleResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// OK
	V3ConnectorScheduleResponse *payments.V3ConnectorScheduleResponse
}

func (v *V3GetConnectorScheduleResponse) GetContentType() string {
	if v == nil {
		return ""
	}
	return v.ContentType
}

func (v *V3GetConnectorScheduleResponse) GetStatusCode() int {
	if v == nil {
		return 0
	}
	return v.StatusCode
}

func (v *V3GetConnectorScheduleResponse) GetRawResponse() *http.Response {
	if v == nil {
		return nil
	}
	return v.RawResponse
}

func (v *V3GetConnectorScheduleResponse) GetV3ConnectorScheduleResponse() *payments.V3ConnectorScheduleResponse {
	if v == nil {
		return nil
	}
	return v.V3ConnectorScheduleResponse
}

// #region class-body-v3getconnectorscheduleresponse
// #endregion class-body-v3getconnectorscheduleresponse
