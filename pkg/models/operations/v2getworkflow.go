// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/formancehq/formance-sdk-go/v3/pkg/models/orchestration"
	"net/http"
)

var V2GetWorkflowServerList = []string{
	"http://localhost:8080/",
}

type V2GetWorkflowRequest struct {
	// The flow id
	FlowID string `pathParam:"style=simple,explode=false,name=flowId"`
}

func (v *V2GetWorkflowRequest) GetFlowID() string {
	if v == nil {
		return ""
	}
	return v.FlowID
}

// #region class-body-v2getworkflowrequest
// #endregion class-body-v2getworkflowrequest

type V2GetWorkflowResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// The workflow
	V2GetWorkflowResponse *orchestration.V2GetWorkflowResponse
}

func (v *V2GetWorkflowResponse) GetContentType() string {
	if v == nil {
		return ""
	}
	return v.ContentType
}

func (v *V2GetWorkflowResponse) GetStatusCode() int {
	if v == nil {
		return 0
	}
	return v.StatusCode
}

func (v *V2GetWorkflowResponse) GetRawResponse() *http.Response {
	if v == nil {
		return nil
	}
	return v.RawResponse
}

func (v *V2GetWorkflowResponse) GetV2GetWorkflowResponse() *orchestration.V2GetWorkflowResponse {
	if v == nil {
		return nil
	}
	return v.V2GetWorkflowResponse
}

// #region class-body-v2getworkflowresponse
// #endregion class-body-v2getworkflowresponse
